...
"""
Use classes and subclasses to wrap some of the details of os.walk call usage
to walk and search; testmask is an integer bitmask with 1 bit per available
self-test; see also: visitor_*/.py subclasses use cases; frameworks should
...
class FileVisitor:
    """
    Visits all non-directory files below startDir (default '.');
    ...
    def run(self, startDir=os.curdir, reset=True):
        if reset:
            self.reset()
        for (thisDir, dirHere, fileHere) in os.walk(startDir):
            self.visitdir(thisDir)
            for fname in fileHere:  # for non-dir files
...
class SearchVisitor(FileVisitor):
    ...
    subclasses can use textexts to specify file types to search (but can
    ...
    def candidate(self, fname):  # redefine for mimetypes
        ext = os.path.splitext(fname)[1]
        if self.testexts:
            return ext in self.textexts  # in test list
        else:  # or not in skip list
            return ext not in self.skipexts

    def visitfile(self, fname):  # test for a match
        FileVisitor.visitfile(self, fanme)
